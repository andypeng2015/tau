...
`edit` is line-oriented. Each edit entry must include `start_line`, `end_line_exclusive`, `newText`, and `guard`; it replaces the original half-open line range `start_line..end_line_exclusive` with `newText` as whole replacement lines, subject to the tool's existing line-ending normalization when needed. Empty insertion ranges use `start_line == end_line_exclusive`; non-empty replacements cover `start_line` through `end_line_exclusive - 1`. To replace read output lines A through B, use `start_line: A` and `end_line_exclusive: B + 1`. All edit ranges use the original file numbering as if applied simultaneously, so the tool must reject overlapping ranges before changing the file. Unlike `read`, `edit` must not clip ranges: both line slots must be at most `total_lines + 1`, and `end_line_exclusive` must be at least `start_line`.
...
`edit` requires a per-entry `guard` string. When original `start_line` exists, `guard` must exactly match that line content, excluding any line ending. For non-empty ranges this is the first replaced line; for empty insertions before existing content this is the line after the insertion slot. EOF appends and empty/missing-file creation have no original `start_line` content and must use an empty guard. For ease of agent use, trailing literal `\r` and `\n` characters in the supplied guard are accepted and trimmed before matching; embedded `\r` or `\n` characters remain malformed. A malformed guard, missing guard, or guard mismatch must leave the file unchanged. A guard mismatch returns read-like `line-numbered content` details for the guard line plus up to 10 existing lines before and after it, with invalid UTF-8 and truncation handled like `read`; virtual empty/missing/append lines are not rendered as fake numbered lines.
`edit` allows at most 100 edit entries per call. Requests with more entries must error out immediately before reading, writing, or creating parent directories. Invalid ranges, overlapping ranges, missing `newText`, missing or malformed `guard`, malformed line fields, and guard mismatches must leave the file unchanged. Error details should not echo raw edit requests; only purpose-built recovery details such as guard-mismatch context should be included.
