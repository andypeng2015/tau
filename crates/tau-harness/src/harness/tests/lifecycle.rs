...
#[test]
fn rendered_agent_prompt_receives_creation_working_directory() {
    let td = TempDir::new().expect("tempdir");
    ...
    h.available_roles
        ...
        .push(tau_config::settings::RolePromptFragment {
            name: "test.cwd.conditional".to_owned(),
            priority: tau_proto::PromptPriority::new(20),
            text: tau_proto::PromptContent::new(
                "{{#if (eq cwd \"/tmp/tau-project\")}}PROJECT={{working_directory.basename}}{{/if}}",
            ),
        });
    let cid = h.create_durable_user_agent(
        "s1".into(),
        &role,
        std::path::PathBuf::from("/tmp/tau-project"),
    );
    let agent_id = durable_agent_id_for_conversation(&h, &cid);

    let prompt = h.build_system_prompt_for_role_and_agent(&role, Some(&agent_id));

    assert!(prompt.contains("PROJECT=tau-project"));
}

#[test]
fn resumed_agent_prompt_restores_working_directory_from_metadata() {
    let td = TempDir::new().expect("tempdir");
    let sp = td.path().join("state");
    let role;
    let agent_id;
    {
        ...
        let cid = h.create_durable_user_agent(
            "s1".into(),
            &role,
            std::path::PathBuf::from("/tmp/resumed-project"),
        );
    ...
    resumed
        ...
        .prompt_fragments
        .push(tau_config::settings::RolePromptFragment {
            name: "test.cwd.resume".to_owned(),
            priority: tau_proto::PromptPriority::new(20),
            text: tau_proto::PromptContent::new(
                "RESUMED={{working_directory.path}}/{{working_directory.basename}}",
            ),
        });

    let prompt = resumed.build_system_prompt_for_role_and_agent(&role, Some(&agent_id));

    assert!(prompt.contains("RESUMED=/tmp/resumed-project/resumed-project"));
