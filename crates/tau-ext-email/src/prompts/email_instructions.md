...
Prefer `list_recent` for normal mailbox review; it searches by IMAP internal date and defaults to the last 7 days. `list_by_uid` is only for raw UID-ordered paging. Both list commands return a `format` header plus one line per message and show access=full|preview|none.
