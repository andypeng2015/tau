...
- `cd` — changes the shell extension's remembered working directory for the current agent. The cwd is also stored as inheritable agent metadata using the extension instance key (for the built-in shell, `ext_core-shell_cwd`).
